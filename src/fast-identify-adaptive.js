...
export async function fastIdentify(request, env) {
  ...
  try {
    ...
    const ai = await verifyAdaptive(env, image, uploadBytes, candidates, timings);
    ...
    timings.total_ms = Date.now() - started;
    const identifiedBy = timings.verification_mode === 'low-fast-path'
      ? 'capa_embedding_topk+gemini-low-fast'
      : 'capa_embedding_topk+gemini-medium-fallback';
