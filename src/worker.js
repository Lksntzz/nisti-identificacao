...
async function getTopCoverCandidates(env, uploadedFile, limit = TOP_K_COVERS) {
  ...
  if (!results?.length) {
    throw new Error('Índice visual vazio. Indexe as imagens das capas antes de identificar.');
  }
  ...
  for (const row of results) {
    try {
      ...
      // Índices corrompidos podem ser refeitos pelo reindexador.
    }
  }
  ...
  return {
    uploadBytes,
    candidates: scored.slice(0, Math.max(1, limit))
  };
}
...
async function verifyCoverWithGemini(env, uploadedFile, uploadBytes, candidates) {
  ...
  const allowedCodes = new Set(
    usableCandidates.map(candidate => String(candidate.capa_code).trim().toUpperCase())
  );
...
async function upsertCatalogProduct(env, row) {
  ...
  let product = await env.DB.prepare(
    `SELECT id,image_key FROM products WHERE sku=?`
    ...
    if (existingPlatform) {
      if (link) {
        await env.DB.prepare(`UPDATE product_platforms SET link=? WHERE id=?`)
          .bind(link, existingPlatform.id).run();
      }
    } else {
      await env.DB.prepare(`
        INSERT INTO product_platforms (product_id,platform,link)
        VALUES (?,?,?)
...
function normalizeMarketplaceImageUrl(value) {
  ...
  try {
    ...
    if (host.endsWith('mlstatic.com')) {
      if (!/\/D_NQ_[A-Z0-9_\-]+/i.test(path)) return null;
    }

    if (host.endsWith('susercontent.com')) {
      if (!/\/file\//i.test(path) && path.split('/').filter(Boolean).length < 2) return null;
    }
...
function collectJsonImages(value, output, depth = 0) {
  if (depth > 12 || output.size >= MAX_LISTING_IMAGES * 3) return;
...
function extractJsonScriptImages(html, output) {
  ...
  for (const script of scripts) {
    if (output.size >= MAX_LISTING_IMAGES * 3) break;
    const body = script
      .replace(/^<script\b[^>]*>/i, '')
      .replace(/<\/script>$/i, '')
      .trim();
    ...
    if (/application\/ld\+json/i.test(script) || body.startsWith('{') || body.startsWith('[')) {
      try {
        ...
        // Estado serializado nem sempre é JSON estrito.
      }
    }
  }
}
...
function extractEmbeddedMarketplaceImages(html, output) {
  ...
  let match;
  while ((match = urlRegex.exec(decoded)) && output.size < MAX_LISTING_IMAGES * 3) {
    const normalized = normalizeMarketplaceImageUrl(match[0]);
    if (normalized) output.add(normalized);
  }
}

function makeListingImageProxyUrl(sourceUrl) {
  return `/api/admin/listing-image?src=${encodeURIComponent(sourceUrl)}`;
}

function unwrapListingImageUrl(value) {
  const text = String(value || '').trim();
  if (!text) return null;

  try {
    if (text.startsWith('/api/admin/listing-image?')) {
      return new URL(text, 'https://nisti.local').searchParams.get('src');
    }

    const parsed = new URL(text);
    if (parsed.pathname === '/api/admin/listing-image') {
...
async function downloadMarketplaceImage(imageUrl) {
  const unwrapped = unwrapListingImageUrl(imageUrl);
  let source;
  try {
    source = new URL(unwrapped);
  } catch {
  ...
  const normalized = normalizeMarketplaceImageUrl(source.toString());
  if (!normalized) throw new Error('A URL não parece ser uma imagem de produto do anúncio');
  ...
  const finalUrl = new URL(response.url || normalized);
  if (!isAllowedMarketplaceImageHost(finalUrl.hostname)) {
    throw new Error('A imagem redirecionou para um domínio não permitido');
  }
  ...
  if (bytes.byteLength < 1024) throw new Error('Arquivo de imagem inválido ou pequeno demais');
...
async function previewMarketplaceListing(listingUrl) {
  ...
  if (!isAllowedMarketplaceHost(finalUrl.hostname)) {
    throw new Error('O anúncio redirecionou para um domínio não permitido');
  }
  ...
  const sources = [...images].slice(0, MAX_LISTING_IMAGES);
  return {
    listing_url: finalUrl.toString(),
    title,
    image_candidates: sources.map(makeListingImageProxyUrl),
...
async function listCoverReviews(env, status, limit, offset) {
  ...
  for (const row of results || []) {
    ...
    if (!cover._productIds.has(row.id)) {
      ...
      if (row.variacao && cover.variacoes.length < 8 && !cover.variacoes.includes(row.variacao)) {
        cover.variacoes.push(row.variacao);
      }
...
async function saveCoverImageFromUrl(env, capaCode, imageUrl) {
  ...
  await env.PRODUCT_IMAGES.put(key, downloaded.bytes, {
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/api/products' && request.method === 'GET') {
        const { results } = await env.DB.prepare(`
          SELECT
            p.id,p.sku,p.miolo_code,p.capa_code,p.acabamento_code,p.wireo_code,p.tassel_code,p.elastico_code,
      ...
      if (url.pathname === '/api/admin/listing-image' && request.method === 'GET') {
        const src = url.searchParams.get('src');
        const downloaded = await downloadMarketplaceImage(src);
      ...
      if (url.pathname === '/api/admin/cover-reviews' && request.method === 'GET') {
        ...
        const status = ['pending', 'ready', 'all'].includes(requestedStatus)
          ? requestedStatus
          : 'pending';
        const limit = Math.max(
          1,
          Math.min(30, Number(url.searchParams.get('limit')) || COVER_REVIEW_PAGE_SIZE)
        );
      ...
      if (coverImageFromUrl && request.method === 'POST') {
        ...
        return json({
          ok: true,
          ...(await saveCoverImageFromUrl(env, capaCode, body?.image_url))
        });
      }
      ...
      if (url.pathname === '/api/admin/cover-index' && request.method === 'GET') {
        const refs = await env.DB.prepare(`
          SELECT COUNT(DISTINCT capa_code) AS total
          FROM products WHERE image_key IS NOT NULL
        `).first();
        ...
        const pending = await env.DB.prepare(`
          SELECT COUNT(*) AS total FROM (
            ...
            LEFT JOIN cover_embeddings ce
              ON ce.capa_code=p.capa_code AND ce.image_key=p.image_key
