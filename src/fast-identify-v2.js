...
// Caminho direto: exige boa similaridade e, principalmente, separação forte
// em relação à segunda capa. Se não passar, o Gemini confirma uma única vez.
const DIRECT_MIN_SCORE = 0.90;
const DIRECT_MIN_MARGIN = 0.05;

// Para a verificação única, casos ainda razoavelmente separados usam LOW.
// Casos realmente próximos usam MEDIUM para preservar detalhe visual.
const LOW_RES_MIN_SCORE = 0.86;
const LOW_RES_MIN_MARGIN = 0.02;
...
function tryDirectEmbedding(candidates, timings) {
  ...
  return {
    ...
    confidence: Math.min(0.999, Math.max(MIN_CONFIDENCE, top.retrieval_score)),
    candidates
  };
}
...
async function verifyOnce(env, image, uploadBytes, candidates, timings) {
  ...
  const mediaResolution = (
    Number.isFinite(topScore) && topScore >= LOW_RES_MIN_SCORE &&
    Number.isFinite(margin) && margin >= LOW_RES_MIN_MARGIN
  ) ? 'MEDIA_RESOLUTION_LOW' : 'MEDIA_RESOLUTION_MEDIUM';

  timings.verification_mode = mediaResolution === 'MEDIA_RESOLUTION_LOW'
    ? 'gemini-single-low'
    : 'gemini-single-medium';
  timings.media_resolution = mediaResolution;
...
export async function fastIdentify(request, env) {
  ...
  try {
    ...
    const ai = direct || await verifyOnce(env, image, uploadBytes, candidates, timings);
    ...
    let identifiedBy = 'capa_embedding+gemini-single-medium';
    if (timings.verification_mode === 'embedding-direct') {
      identifiedBy = 'capa_embedding-direct';
    } else if (timings.verification_mode === 'gemini-single-low') {
      identifiedBy = 'capa_embedding+gemini-single-low';
    }
