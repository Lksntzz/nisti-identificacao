...
<head>
  ...
  <title>NISTI · Administração</title>
  <style>
    *{box-sizing:border-box}body{margin:0;font-family:Inter,Arial,sans-serif;background:#f3f4f6;color:#111827;min-height:100vh;display:grid;place-items:center;padding:20px}
    .card{width:min(420px,100%);background:#fff;border:1px solid #e5e7eb;border-radius:22px;padding:28px;box-shadow:0 18px 50px rgba(17,24,39,.10)}
    .brand{font-size:11px;font-weight:900;letter-spacing:.16em;color:#6b7280;margin:0 0 8px}.card h1{font-size:28px;margin:0 0 8px}.card p{color:#6b7280;line-height:1.5;margin:0 0 20px}
    label{display:grid;gap:8px;font-size:13px;font-weight:800;color:#374151}input{width:100%;padding:14px 15px;border:1px solid #d1d5db;border-radius:12px;font:inherit;outline:none}input:focus{border-color:#111827;box-shadow:0 0 0 3px rgba(17,24,39,.08)}
    button{width:100%;margin-top:14px;border:0;border-radius:12px;padding:14px 16px;font:inherit;font-weight:900;background:#111827;color:#fff;cursor:pointer}.error{padding:11px 12px;border:1px solid #fecaca;background:#fef2f2;color:#991b1b;border-radius:10px;margin-bottom:16px;font-size:13px}.back{display:block;text-align:center;margin-top:16px;color:#6b7280;text-decoration:none;font-size:13px}
  </style>
</head>
<body>
  <main class="card">
    ...
    <p>Acesso restrito. Somente pessoas autorizadas podem abrir o painel administrativo.</p>
    ...
    <a class="back" href="/">Voltar ao Painel Geral</a>
  </main>
</body>
</html>`;
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (pathname === '/admin-login' && request.method === 'POST') {
      ...
      if (!configured) {
        return html(loginPage('A administração está bloqueada até o segredo ADMIN_PASSWORD ser configurado no Cloudflare.'), 503);
      }
