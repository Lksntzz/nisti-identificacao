...
async function loadCandidateImage(env, candidate, platform) {
  let row = null;
...
async function compareCatalogCandidates(env, photoBytes, photoMime, candidates, platform) {
  ...
  const allowed = await reserveGeminiBudget(env, 'gemini-3.5-flash-lite-catalog-comparator-v7', VERIFIER_RPM_LIMIT);
  if (!allowed) {
...
Nenhuma candidata é referência principal. Analise TODAS de forma independente e descreva a evidência estrutural de cada uma. O código da aplicação decidirá o vencedor; não transforme "mais parecida" em identidade.
...
- Quando PERSONALIZADO=SIM, nome próprio, inicial/letra ou data presentes no mockup são conteúdo variável. Eles NÃO podem reprovar uma arte-base quando os elementos permanentes coincidirem.
- Quando PERSONALIZADO=NÃO, não descarte texto visível como se fosse nome variável sem evidência clara.
- Títulos, frases, categoria do produto e textos que fazem parte do desenho fixo nunca devem ser ignorados.
- personalization_difference_only=true somente quando a divergência textual for exclusivamente conteúdo variável de personalização e o restante da arte coincidir.

IGNORE Wire-O/espiral, elástico, tassel, mão, mesa, reflexo, brilho, perspectiva, recorte e fundo externo.
Não aceite por tema ou cor parecida. Marque disqualifying_conflict=true quando houver diferença permanente importante.`;
  ...
  for (const candidate of candidates) {
    ...
    parts.push({ inline_data: { mime_type: candidate.mime_type, data: base64(candidate.bytes) } });
  }
...
export async function structuralFinalIdentifyV7(request, env) {
  const started = Date.now();
  const performance = {
    pipeline_version: 'platform-vectorize+comparative-catalog-v7',
    verification_mode: 'catalog-evidence+deterministic-adjudication',
  ...
  try {
    ...
    const loaded = (await Promise.all(candidates.map(candidate => loadCandidateImage(env, candidate, platform))))
      .filter(Boolean);
    ...
    if (!winner) {
      ...
      performance.accepted_by = comparison ? 'deterministic-adjudication-rejected' : 'comparator-unavailable-safe-suggestions';
      ...
      return json({
        ...
        identified_by: suggestions.length ? 'platform-catalog-visual-suggestions-v7' : 'platform-catalog-no-match-v7',
        performance
      }, 422);
    }
    ...
    if (products.length > 1) {
      return json({
        ...
        identified_by: 'platform-catalog-v7+human-sku-selection',
        performance
      });
    }
    ...
    return json({
      ...
      confidence: winner.assessment.confidence,
      personalization_detected: comparison.personalization_detected,
      identified_by: 'platform-catalog-v7-deterministic-winner',
      performance
    });
  } catch (error) {
    finalizePerformance(performance, started);
