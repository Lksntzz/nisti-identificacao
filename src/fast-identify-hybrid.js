...
const DIRECT_MIN_SCORE = 0.96;
const DIRECT_MIN_MARGIN = 0.05;
const FAST_CONFIDENCE = 0.95;
const FAST_MIN_MARGIN = 0.01;
...
async function getCandidates(env, image, timings) {
  ...
  timings.retrieval_margin = candidates.length > 1
    ? candidates[0].retrieval_score - candidates[1].retrieval_score
    : null;
  return { uploadBytes, candidates };
}

function directEmbeddingMatch(candidates, timings) {
  const top = candidates[0];
  const second = candidates[1];
  if (!top?.capa_code || !Number.isFinite(top.retrieval_score)) return null;

  const margin = second && Number.isFinite(second.retrieval_score)
    ? top.retrieval_score - second.retrieval_score
    : 1;

  timings.embedding_direct_score_threshold = DIRECT_MIN_SCORE;
  timings.embedding_direct_margin_threshold = DIRECT_MIN_MARGIN;
  timings.embedding_direct_eligible = Boolean(
    top.retrieval_score >= DIRECT_MIN_SCORE && margin >= DIRECT_MIN_MARGIN
  );

  if (!timings.embedding_direct_eligible) return null;

  timings.verification_mode = 'embedding-direct';
  return {
    matched: true,
    capa_code: String(top.capa_code).trim().toUpperCase(),
    confidence: Math.min(0.999, Math.max(MIN_CONFIDENCE, Number(top.retrieval_score))),
    candidates
  };
}
...
function buildParts(image, uploadBytes, usable) {
  ...
  for (const candidate of usable) {
    parts.push({
      text: `CAPA CANDIDATA: CAPA_CODE=${candidate.capa_code}; retrieval_rank=${candidate.retrieval_rank}; retrieval_score=${candidate.retrieval_score.toFixed(6)}`
    });
    parts.push({
      inline_data: {
...
async function callGemini(env, image, uploadBytes, usable, mediaResolution) {
  ...
  if (result?.matched && (!capaCode || !allowed.has(capaCode))) {
    return { matched: false, capa_code: '', confidence: 0 };
  }
  return { ...result, capa_code: capaCode };
}

async function verifyAdaptive(env, image, uploadBytes, candidates, timings) {
  const usable = await loadCandidates(env, candidates, timings);
  const topCandidateCode = String(candidates[0]?.capa_code || '').trim().toUpperCase();
  const retrievalMargin = Number(timings.retrieval_margin);

  const fastStarted = Date.now();
  const fast = await callGemini(env, image, uploadBytes, usable, 'MEDIA_RESOLUTION_LOW');
  timings.gemini_fast_ms = Date.now() - fastStarted;
  timings.gemini_fast_confidence = Number(fast?.confidence) || 0;

  const fastAgreement = Boolean(
    fast?.matched &&
    fast?.capa_code &&
    String(fast.capa_code).toUpperCase() === topCandidateCode &&
    Number(fast.confidence) >= FAST_CONFIDENCE &&
    Number.isFinite(retrievalMargin) &&
    retrievalMargin >= FAST_MIN_MARGIN
  );

  if (fastAgreement) {
    timings.verification_mode = 'low-fast-path';
    return { ...fast, candidates: usable };
  }

  const fullStarted = Date.now();
  const full = await callGemini(env, image, uploadBytes, usable, 'MEDIA_RESOLUTION_MEDIUM');
  timings.gemini_fallback_ms = Date.now() - fullStarted;
  timings.verification_mode = 'medium-fallback';
  return { ...full, candidates: usable };
}
...
function timingHeader(timings) {
  const entries = [
    ...
    ['gemini_fast', timings.gemini_fast_ms],
    ['gemini_fallback', timings.gemini_fallback_ms],
...
export async function fastIdentify(request, env) {
  ...
  try {
    ...
    const direct = directEmbeddingMatch(candidates, timings);
    const ai = direct || await verifyAdaptive(env, image, uploadBytes, candidates, timings);
    ...
    timings.total_ms = Date.now() - started;
    let identifiedBy = 'capa_embedding_topk+gemini-medium-fallback';
    if (timings.verification_mode === 'embedding-direct') {
      identifiedBy = 'capa_embedding-direct';
    } else if (timings.verification_mode === 'low-fast-path') {
      identifiedBy = 'capa_embedding_topk+gemini-low-fast';
    }
