...
function referenceFromVectorMatch(match) {
...
async function vectorCandidates(env, vector) {
  ...
  for (const match of result?.matches || []) {
    ...
    const reference = referenceFromVectorMatch(match);
    if (reference && cover.references.length < REFERENCES_PER_COVER && !cover.references.some(item => item.reference_id === reference.reference_id)) {
      reference.vector_rank = vectorRank;
      cover.references.push(reference);
    }
  }

  const covers = [...byCode.values()].slice(0, COVER_LIMIT);
...
async function loadReferences(env, covers) {
  ...
  for (const cover of covers) {
    ...
    for (const candidate of cover.references || []) {
      const row = byId.get(Number(candidate.reference_id));
      if (!row || row.capa_code !== cover.capa_code || chosen.some(item => item.row.id === row.id)) continue;
      chosen.push({ row, candidate, exact: true });
    ...
    if (chosen.length < REFERENCES_PER_COVER) {
      for (const row of byCode.get(cover.capa_code) || []) {
        if (chosen.some(item => item.row.id === row.id)) continue;
        chosen.push({
          row,
          candidate: {
            reference_id: row.id,
    ...
    chosen.forEach((item, index) => selected.push({
      ...item.row,
      retrieval_rank: cover.retrieval_rank,
      retrieval_score: Number(item.candidate.retrieval_score ?? cover.retrieval_score ?? 0),
      vector_rank: item.candidate.vector_rank || null,
      reference_rank: index + 1,
      exact_retrieval_reference: item.exact
    }));
  }
...
Pode haver mais de uma REFERÊNCIA para o mesmo CAPA_CODE. Trate essas imagens como diferentes aparências da mesma arte-base e combine a evidência entre elas.
...
Responda somente no JSON solicitado.`
  ...
  for (const ref of references) {
    parts.push({
      text: `REFERÊNCIA ${ref.retrieval_rank}.${ref.reference_rank}: CAPA_CODE=${ref.capa_code}; reference_id=${ref.id}; tipo=${ref.reference_kind}; score=${Number(ref.retrieval_score || 0).toFixed(6)}`
    });
    parts.push({
      inline_data: {
...
function finalizePerformance(performance, started) {
  ...
  performance.total_ms = performance.reused_candidates
    ? Math.max(0, Number(performance.candidate_generation_ms || 0)) + fallbackMs
    : fallbackMs;
}

export async function structuralFallbackIdentifyV4(request, env) {
  const started = Date.now();
  const deadlineAt = started + TOTAL_BUDGET_MS;
  const performance = {
    pipeline_version: 'vectorize-multiref-ticket+gemini-structural-v4',
    verification_mode: 'gemini-multi-reference-structured-json'
  };
  ...
  try {
    ...
    if (covers.length) {
      ...
      performance.retrieval_source = 'vectorize-ticket-multiref-reuse';
      performance.reused_candidates = true;
    } else {
      performance.retrieval_source = 'vectorize-fallback-multiref';
      performance.reused_candidates = false;
      const embeddingStarted = Date.now();
      const embedding = await embedImage(env, bytes, image.type || 'image/jpeg', deadlineAt);
      performance.embedding_ms = Date.now() - embeddingStarted;
      performance.model = embedding.model;
      const vectorStarted = Date.now();
      covers = await vectorCandidates(env, embedding.values);
      performance.vectorize_ms = Date.now() - vectorStarted;
      performance.vector_top_k = VECTOR_TOP_K;
    }

    performance.retrieval_top1_code = performance.retrieval_top1_code || covers[0]?.capa_code || null;
    performance.retrieval_top1 = performance.retrieval_top1 ?? covers[0]?.retrieval_score ?? null;
    performance.retrieval_top2_code = performance.retrieval_top2_code || covers[1]?.capa_code || null;
    performance.retrieval_top2 = performance.retrieval_top2 ?? covers[1]?.retrieval_score ?? null;
    performance.retrieval_margin = performance.retrieval_margin ?? (
      covers.length > 1
    ...
    if (!verification.matched || !verification.capa_code) {
      ...
      return json({
        ...
        identified_by: 'vectorize-ticket+gemini-multireference',
        performance
      }, 422);
    }
    ...
    performance.accepted_by = 'vectorize-ticket+gemini-multireference';
    finalizePerformance(performance, started);
    ...
    if (products.length > 1) {
      return json({
        ...
        confidence: verification.confidence,
        identified_by: 'vectorize-ticket+gemini-multireference+human-sku-selection',
        performance
      });
    }
