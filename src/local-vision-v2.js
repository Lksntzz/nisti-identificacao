...
const FAST_SIDE = 480;
const DETAIL_SIDE = 700;
const FAST_FEATURES = 560;
const DETAIL_FEATURES = 900;
const MAX_CANDIDATES = 8;
const PREFETCH_CANDIDATES = 4;

// O servidor repete estes limites antes de liberar o SKU. Mantemos o cliente
// no mesmo nível para evitar falso positivo e usamos a segunda passada apenas
// para aumentar recall em artes suaves/florais.
...
export function warmLocalVision() {
  // JSFeat já está no bundle; não existe runtime WASM para inicializar.
}
...
function detectReferenceCrop(bitmap) {
  ...
  for (let y = 0; y < height; y += 2) {
    for (let x = 0; x < width; x += 2) {
      ...
      // Mockups têm fundo branco/cinza. O limiar preserva artes muito claras e
      // remove apenas a área vazia em volta do produto.
      const nonWhite = r < 246 || g < 246 || b < 246 || spread > 10;
...
function extractFeatures(canvas, { maxFeatures, detail = false } = {}) {
  ...
  // A passada detalhada reduz os limiares para capturar pétalas, ramos e textos
  // finos sem relaxar a validação geométrica final.
...
function matchFeatureSets(reference, photo, detail = false) {
  ...
  const referencePoints = matches.map(match => ({ x: reference.corners[match.trainIdx].x, y: reference.corners[match.trainIdx].y }));
  const photoPoints = matches.map(match => ({ x: photo.corners[match.queryIdx].x, y: photo.corners[match.queryIdx].y }));
  ...
  const ok = jsfeat.motion_estimator.ransac(params, kernel, referencePoints, photoPoints, goodMatches, homography, mask, detail ? 700 : 450);
  if (!ok) {
    return { accepted: false, strong: false, good_matches: goodMatches, inliers: 0, inlier_ratio: 0, median_distance: median(matches.map(m => m.distance)), geometric_score: 0, geometry_sane: false };
  }
...
async function referenceFeatures(candidate, mode) {
  ...
  const maxSide = detail ? DETAIL_SIDE : FAST_SIDE;
  const maxFeatures = detail ? DETAIL_FEATURES : FAST_FEATURES;
  const key = `${candidate.image_url}|${mode}`;
  if (referenceFeatureCache.has(key)) return referenceFeatureCache.get(key);
  const promise = (async () => {
    const bitmap = await bitmapFromUrl(candidate.image_url);
    try {
      return extractFeatures(canvasFromBitmap(bitmap, { reference: true, maxSide }), { maxFeatures, detail });
    } finally {
      bitmap.close?.();
    }
  })().catch(error => {
  ...
  referenceFeatureCache.set(key, promise);
  // Evita crescimento sem limite em sessões longas de operação.
  if (referenceFeatureCache.size > 48) {
    const oldest = referenceFeatureCache.keys().next().value;
    if (oldest) referenceFeatureCache.delete(oldest);
  }
  return promise;
}
...
function resultFromTested(tested, started, runtimeError = null, pass = 'fast') {
  ...
  return {
    ...
    ambiguous,
    runner: `jsfeat-orb-ransac-v2-${pass}`,
...
async function testPass(photo, entries, mode, started, deadlineMs, tested) {
  const detail = mode === 'detail';
  for (const entry of entries) {
    if (now() - started >= deadlineMs) break;
    try {
      const reference = await referenceFeatures(entry.candidate, mode);
      const metrics = matchFeatureSets(reference, photo, detail);
      tested.push({ ...entry.candidate, ...metrics, pass: mode });
      if (metrics.strong) break;
    } catch (error) {
      tested.push({ ...entry.candidate, accepted: false, pass: mode, error: error?.message || 'Falha ao comparar referência' });
    }
  }
}
...
export async function matchLocalCandidates(photoFile, candidates, options = {}) {
  const started = now();
  const deadlineMs = Math.max(1000, Number(options.deadlineMs) || 2800);
  const allCandidates = (candidates || []).slice(0, MAX_CANDIDATES);
  const tested = [];
  let photoBitmap;

  try {
    // Dispara as referências mais prováveis em paralelo enquanto processa a foto.
    for (const candidate of allCandidates.slice(0, PREFETCH_CANDIDATES)) referenceFeatures(candidate, 'fast').catch(() => {});

    photoBitmap = await bitmapFromBlob(photoFile);
    const fastPhoto = extractFeatures(canvasFromBitmap(photoBitmap, { maxSide: FAST_SIDE }), { maxFeatures: FAST_FEATURES, detail: false });
    const fastEntries = allCandidates.slice(0, 5).map(candidate => ({ candidate }));
    await testPass(fastPhoto, fastEntries, 'fast', started, Math.min(deadlineMs, 1300), tested);

    let choice = chooseResult(tested);
    if (choice.accepted && (choice.accepted.strong || now() - started >= deadlineMs * 0.65)) {
      return resultFromTested(tested, started, null, 'fast');
    }

    // Capas florais e artes com traços finos perdem detalhes a 480 px. Se a
    // primeira passada não fechou, reprocessa a foto em maior resolução e
    // verifica todas as 8 candidatas que o embedding já retornou.
    if (now() - started < deadlineMs) {
      for (const candidate of allCandidates.slice(0, PREFETCH_CANDIDATES)) referenceFeatures(candidate, 'detail').catch(() => {});
      const detailPhoto = extractFeatures(canvasFromBitmap(photoBitmap, { maxSide: DETAIL_SIDE }), { maxFeatures: DETAIL_FEATURES, detail: true });
      const detailEntries = allCandidates.map(candidate => ({ candidate }));
      const detailTested = [];
      await testPass(detailPhoto, detailEntries, 'detail', started, deadlineMs, detailTested);
      // A decisão final usa a passada detalhada quando ela encontrou qualquer
      // candidato válido; caso contrário mantém os dados da primeira passada
      // para diagnóstico do ADM.
