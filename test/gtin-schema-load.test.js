...
test('GTIN schema and seed remain separated and locked to 220 approved links', () => {
  ...
  const d1Seed = fs.readFileSync('migrations/0016_product_gtins_seed_v1.sql', 'utf8');
  ...
  assert.equal((d1Seed.match(/^\s*\(\d+, '\d{13}'/gm) || []).length, 220);
});
