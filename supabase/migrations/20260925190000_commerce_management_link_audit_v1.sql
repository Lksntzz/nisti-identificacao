...
rows as (
  select
    ...
    lower(regexp_replace(btrim(coalesce(r.normalized_payload->>'product_name','')),'[^[:alnum:]]+','','g')) as name_norm,
    upper(btrim(coalesce(r.normalized_payload->>'category',''))) as category_norm,
    coalesce(
...
candidates as (
  select
    u.source_row_id,
    min(l.matched_product_id) as candidate_product_id,
    count(distinct l.matched_product_id) as candidate_count
  from rows u
  join rows l
    on l.matched_product_id is not null
   and l.name_norm=u.name_norm and l.name_norm<>''
...
safe as (
  select source_row_id,candidate_product_id
  from candidates
  where candidate_count=1
)
update public.commerce_preview_source_rows r
set matched_product_id=s.candidate_product_id,
    resolution_status='PRODUCT_MATCHED',
    notes=concat_ws(' · ',nullif(r.notes,''),'preview auto-link: exact name + category + year')
from safe s
...
selected_files as (
  select s.code,(
    select f.id from public.commerce_source_files f
...
rows as (
  ...
  join public.commerce_source_rows r on r.source_file_id=sf.source_file_id and r.is_header=false
),
presence as (
  select product_id,count(distinct code)::integer as platform_count
  from rows where product_id is not null group by product_id
),
candidates as (
  select
    u.source_row_id,
...
select jsonb_build_object(
  ...
  'safe_candidates',(select count(*) from candidates where candidate_count=1),
  'ambiguous_candidates',(select count(*) from candidates where candidate_count>1),
  'no_safe_candidate',(select count(*) from candidates where candidate_count=0)
);
$function$;
...
CREATE OR REPLACE FUNCTION public.commerce_management_products_v2(p_search text DEFAULT NULL::text, p_category text DEFAULT NULL::text, p_year integer DEFAULT NULL::integer, p_presence text DEFAULT 'LINKED'::text, p_limit integer DEFAULT 24, p_offset integer DEFAULT 0)
...
linked_cards as (
  select
    ...
    lb.presence_type,
    lb.platforms,
    'LINKED'::text as link_review_status,
...
unlinked_cards as (
  select
    ...
    case
      when coalesce(uc.candidate_count,0)=1 then 'SAFE_CANDIDATE'
...
$function$;
revoke execute on function public.commerce_management_products_v2(text,text,integer,text,integer,integer) from public, anon, authenticated;
grant execute on function public.commerce_management_products_v2(text,text,integer,text,integer,integer) to service_role;
...
selected_files as (
  select s.code,(
    select f.id from public.commerce_preview_source_files f
    where upper(f.source_code) in (s.code,s.code||'_GESTAO')
    order by case when upper(f.source_code)=s.code||'_GESTAO' then 0 else 1 end,
             f.imported_at desc nulls last,f.id desc
    limit 1
