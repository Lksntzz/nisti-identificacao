...
-- Regra de autoridade do ano da arte:
-- * Shopee: o título visível pode refletir a arte já atualizada antes do SKU.
-- * Demais marketplaces: o SKU anual é a autoridade quando contém ano.
      ...
      join lateral (
        ...
        where lp2.product_id=target.product_id
          ...
          and public.commerce_image_years_compatible(
            ...
            lp2.platform_sku,
            case when upper(m.code)='SHOPEE' then coalesce(s.title,l2.title) else null end
          )
      ...
      left join lateral (
        ...
        where src_lp.product_id=lp.product_id
          ...
          and public.commerce_image_years_compatible(
            lp.platform_sku,
            src_lp.platform_sku,
            case when upper(src_marketplace.code)='SHOPEE' then coalesce(src_snapshot.title,src_listing.title) else null end
          )
    ...
    where s.match_status='MATCHED'
      ...
      and (
        ...
        or public.commerce_image_years_compatible(
          ...
          lp.platform_sku,
          case when upper(m.code)='SHOPEE' then coalesce(s.title,l.title) else null end
        )
      )
    order by
      case
