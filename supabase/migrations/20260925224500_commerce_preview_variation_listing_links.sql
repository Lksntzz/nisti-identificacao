...
-- Preview-only: model PTD180_EPND_BBB as one Shopee parent listing with three concrete Product Master variations.
do $$
declare
  v_marketplace_id bigint;
  v_listing_id bigint;
  v_source_row_id bigint;
  v_next_listing_id bigint;
  v_next_link_id bigint;
  v_rec record;
begin
  ...
  if v_marketplace_id is null then
    raise exception 'Shopee marketplace not found in preview';
  end if;

  select r.id into v_source_row_id
  from public.commerce_preview_source_rows r
  join public.commerce_preview_source_files f on f.id=r.source_file_id
  where upper(f.source_code) in ('SHOPEE','SHOPEE_GESTAO')
    and regexp_replace(
      upper(coalesce(nullif(r.normalized_payload->>'sku',''),nullif(r.normalized_payload->>'sku_base',''),'')),
      '[^A-Z0-9]+','','g'
    )='PTD180EPNDBBB'
  order by f.imported_at desc nulls last,f.id desc,r.id desc
  limit 1;

  if v_source_row_id is null then
    raise exception 'PTD180_EPND_BBB parent source row not found';
  end if;

  select id into v_listing_id
  from public.commerce_preview_listings
  where marketplace_id=v_marketplace_id
    and external_listing_id='53017814879'
  order by id
  limit 1;

  if v_listing_id is null then
    select coalesce(max(id),0)+1 into v_next_listing_id
    from public.commerce_preview_listings;

    insert into public.commerce_preview_listings(
      id,marketplace_id,external_listing_id,canonical_url,source_url,title,
      listing_status,sales_status,video_status,observed_year,source,last_checked_at,raw_metadata,
      created_at,updated_at
    ) values (
      v_next_listing_id,
      v_marketplace_id,
      '53017814879',
      'https://shopee.com.br/product/376221706/53017814879/',
      'https://shopee.com.br/product/376221706/53017814879/',
      'Nisti Print Caderno de anotações Espanhol Capa Dura Atualizado - Dalia',
      'UNKNOWN','UNKNOWN','UNKNOWN',null,'EXCEL',now(),
      jsonb_build_object(
        'parent_sku','PTD180_EPND_BBB',
        'source_row_id',v_source_row_id,
        'variation_family','EPND',
        'preview_only',true
      ),
      now(),now()
    );

    v_listing_id:=v_next_listing_id;
  end if;

  select coalesce(max(id),0)+1 into v_next_link_id
  ...
  for v_rec in
    select
      ...
      ps.sku,
      case
        when regexp_replace(upper(ps.sku),'[^A-Z0-9]+','','g')='PTD180EPND1BBB' then 'CAPA 1'
        when regexp_replace(upper(ps.sku),'[^A-Z0-9]+','','g')='PTD180EPND2BBB' then 'CAPA 2'
        when regexp_replace(upper(ps.sku),'[^A-Z0-9]+','','g')='PTD180EPND3BBB' then 'CAPA 3'
      end as variation_name
    from public.commerce_preview_product_skus ps
    where regexp_replace(upper(ps.sku),'[^A-Z0-9]+','','g')
      in ('PTD180EPND1BBB','PTD180EPND2BBB','PTD180EPND3BBB')
    order by ps.sku
  loop
    if not exists (
      select 1
      ...
      where lp.listing_id=v_listing_id
        and lp.product_id=v_rec.product_id
    ) then
      insert into public.commerce_preview_listing_products(
        id,listing_id,product_id,product_sku_id,platform_sku,variation_id,variation_name,
        relation_status,created_at,updated_at
      ) values (
        v_next_link_id,v_listing_id,v_rec.product_id,v_rec.product_sku_id,v_rec.sku,
        null,v_rec.variation_name,'ACTIVE',now(),now()
      );
      v_next_link_id:=v_next_link_id+1;
    end if;
  end loop;

  if (
    select count(distinct lp.product_id)
    from public.commerce_preview_listing_products lp
    where lp.listing_id=v_listing_id
      and lp.relation_status='ACTIVE'
      and regexp_replace(upper(coalesce(lp.platform_sku,'')),'[^A-Z0-9]+','','g')
        in ('PTD180EPND1BBB','PTD180EPND2BBB','PTD180EPND3BBB')
  )<>3 then
    raise exception 'EPND parent listing must resolve exactly 3 Product Master variations';
  end if;

  update public.commerce_preview_source_rows
  set matched_product_id=null,
      matched_listing_id=v_listing_id,
      resolution_status='VARIATION',
      notes=concat_ws(
        ' · ',
        nullif(notes,''),
        'preview parent listing: PTD180_EPND_BBB maps to EPND1/EPND2/EPND3 via listing variations'
      )
  where id=v_source_row_id;
end $$;

-- MPO is the cover identity. Keep conflicting marketplace color/year labels at the item level.
update public.commerce_preview_products p
set name='Planner Semanal e Mensal - My Planner - MPO',
    notes=concat_ws(
      ' · ',
      nullif(p.notes,''),
      'Amazon usa título Orange; Shopee usa White. Mesmo cover code MPO; cor permanece atributo da listagem, não identidade do mestre.'
    ),
    updated_at=now()
where exists (
  select 1
  ...
  where ps.product_id=p.id
    and regexp_replace(upper(ps.sku),'[^A-Z0-9]+','','g')='PLAN26MPOPBP'
);
