...
with latest_file as (
  ...
  where upper(f.source_code)=upper(btrim(coalesce(p_source_code,'')))
...
resolved as (
  select n.*,
    coalesce(direct_image.image_url,inherited_image.image_url,nisti_image.image_url) as resolved_image_url,
    case when direct_image.image_url is not null then 'MARKETPLACE'
    ...
    case when direct_image.image_url is not null then direct_image.marketplace_code
    ...
    case when direct_image.image_url is not null then direct_image.marketplace_name
    ...
    case when direct_image.image_url is not null then direct_image.platform_sku
  ...
  left join lateral (
    ...
    limit 1
  ) inherited_image on direct_image.image_url is null
  left join lateral (
  ...
  ) nisti_image on direct_image.image_url is null and inherited_image.image_url is null
)
select
