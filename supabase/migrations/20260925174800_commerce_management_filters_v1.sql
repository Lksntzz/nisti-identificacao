...
CREATE OR REPLACE FUNCTION public.commerce_management_filter_options_v1(p_source_code text DEFAULT 'AMAZON'::text)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
AS $function$
with first_page as (
  select * from public.commerce_management_rows_v1(
...
$function$;
revoke execute on function public.commerce_management_filter_options_v1(text) from public, anon, authenticated;
grant execute on function public.commerce_management_filter_options_v1(text) to service_role;
